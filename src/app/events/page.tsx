...
const SAMPLE_EVENTS: Event[] = [
  {
    ...
    coordinates: { lat: 59.94, lng: 10.7217 },
  },
  {
    id: '2',
    ...
    <div className="min-h-screen">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <div>
            <h2 className="text-2xl font-bold label mb-2">Kommende arrangementer</h2>
            <p className="muted">
            ...
            <div className="flex panel rounded-lg p-1">
              <button
                ...
                className={`px-4 py-2 rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                  viewMode === 'list' ? 'bg-panel label shadow-sm' : 'muted'
                }`}
              >
                📋 Liste
              </button>
              <button
                ...
                className={`px-4 py-2 rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                  viewMode === 'map' ? 'bg-panel label shadow-sm' : 'muted'
                }`}
              >
                🗺️ Kart
              </button>
        ...
        {filteredEvents.length === 0 ? (
          <div className="text-center py-16">
            <p className="text-xl muted">Ingen arrangementer funnet med disse filtrene</p>
            <p className="muted mt-2">Prøv å endre filtrene eller legg til et nytt arrangement!</p>
          </div>
        ) : viewMode === 'map' ? (
      ...
      {showAddForm && <AddEventForm onClose={() => setShowAddForm(false)} onAdd={handleAddEvent} />}
